<?php
		...
		$outputAll="";
	  	...
	  	for($page=$start;$page<=$end;$page++){
	  		...
	  		$call = "http://api.redtube.com/?data=redtube.Videos.searchVideos&output=json&search=$keyword&tags[]=$tagv&thumbsize=medium2&page=$page";
			...
			foreach($videos as $video) {
				...
				$title = addslashes($video_obj->title);
				$url = $video_obj->url;
				...
				if($num_rows>0){
					...
					//check thumbs valid
					foreach($video_obj->thumbs as $val){
					...
					if(!$errorInsert){
						...
						$str_time = $video_obj->duration;
						$str_time = preg_replace("/^([\d]{1,2})\:([\d]{2})$/", "00:$1:$2", $str_time);
						sscanf($str_time, "%d:%d:%d", $hours, $minutes, $seconds);
						$duration = $hours * 3600 + $minutes * 60 + $seconds;
									
						if(!empty($video_obj->tags)){

							$tags = "";
							foreach($video_obj->tags as $tag){
						...
						if(!empty($video_obj->stars)){

							$performersArr = array();
							foreach($video_obj->stars as $star){
