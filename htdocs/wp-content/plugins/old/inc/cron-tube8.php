...
if(!empty($_POST['Delete-tube8'])) {
	...
	foreach( $id1 as $cron_name) {
		...
		if($del[$cron_name]==1){
			...
			$existingCronNames = get_site_option('tubeace_cron_tube8_'.$frequency);
			...
			if(count($existingCronNames)>1){
				...
				update_site_option("tubeace_cron_tube8_".$frequency, $existingCronNames);
			
			} else { //only one name, delete from options and deactivate group cron hook


				//delete empty group option_name
				$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_tube8_".$frequency ) );
			...
			$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_tube8_".$cron_name."_".$frequency ) );
	...
	if($i>0){
		...
		echo"<div class=\"error\"><p><b>No Tube8 Cron Jobs selected to delete.</b></p></div>";
	}
}

?>

<h3>Tube8 Import Cron Jobs</h3>
...
<table class="widefat" cellspacing="0">
  <thead>
	...
	<tbody>
		...
		//hourly
		//get options array
		$existingCronNamesHourly = get_site_option('tubeace_cron_tube8_hourly');
		...
		if(!empty($existingCronNamesHourly)){
			...
			foreach($existingCronNamesHourly as $name){
				...
				if(!in_array($name, $deleted_cron_names)){
				    ...
				    $option_values = get_site_option('tubeace_cron_tube8_'.$name.'_hourly');
					...
					<tr class=\"alternate\">
						...
						<td>Hourly <br><a href=\"admin.php?page=tubeace/tubeace-cron-tube8-edit.php&freq=hourly&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$start</td>
						<td>$end</td>
		...
		//twicedaily
		//get options array
		$existingCronNamesTwice = get_site_option('tubeace_cron_tube8_twicedaily');
		...
		if(!empty($existingCronNamesTwice)){
			...
			foreach($existingCronNamesTwice as $name){
				...
				if(!in_array($name, $deleted_cron_names)){				
				    ...
				    $option_values = get_site_option('tubeace_cron_tube8_'.$name.'_twicedaily');
					...
					<tr class=\"alternate\">
						...
						<td>Twice Daily <br><a href=\"admin.php?page=tubeace/tubeace-cron-tube8-edit.php&freq=twicedaily&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$start</td>
						<td>$end</td>
		...
		//daily
		//get options array
		$existingCronNamesDaily = get_site_option('tubeace_cron_tube8_daily');
		...
		if(!empty($existingCronNamesDaily)){
			...
			foreach($existingCronNamesDaily as $name){
				...
				if(!in_array($name, $deleted_cron_names)){
				    ...
				    $option_values = get_site_option('tubeace_cron_tube8_'.$name.'_daily');
					...
					<tr class=\"alternate\">
						...
						<td>Daily <br><a href=\"admin.php?page=tubeace/tubeace-cron-tube8-edit.php&freq=daily&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$start</td>
						<td>$end</td>
...
<input type="submit" value="Delete Selected" class="button-primary" name="Delete-tube8">
</form>