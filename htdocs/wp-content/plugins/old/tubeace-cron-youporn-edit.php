...
if( isset($_POST['Submit']) ){
	...
	update_site_option("tubeace_cron_youporn_".$cron_name."_".$cron_frequency, $cronValues);
	...
	if($current_frequency != $cron_frequency){
		...
		$existingCronNames = get_site_option('tubeace_cron_youporn_'.$current_frequency);
		...
		if(count($existingCronNames)>1){
			...
			update_site_option("tubeace_cron_youporn_".$current_frequency, $existingCronNames);
		
		} else { //only one name, delete from options and deactivate group cron hook

			//delete empty group option_name
			$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_youporn_".$current_frequency ) );
		...
		$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_youporn_".$cron_name."_".$current_frequency ) );

		//create new frequency group, or add to existing
		$newCronNames = get_site_option('tubeace_cron_youporn_'.$cron_frequency);
		...
		update_site_option("tubeace_cron_youporn_".$cron_frequency, $newCronNames);

		//rename set all's option_name
		$wpdb->update( 'options', array('option_name' => "tubeace_cron_youporn_".$cron_name."_".$cron_frequency ), array('option_name' => "tubeace_cron_youporn_".$cron_name."_".$current_frequency) );
...
$option_values = get_site_option('tubeace_cron_youporn_'.$cron_name.'_'.$cron_frequency);

?>

<h2>Edit YouPorn Import Cron Job</h2>

<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-cron-youporn-edit.php'); ?>" method="POST">
