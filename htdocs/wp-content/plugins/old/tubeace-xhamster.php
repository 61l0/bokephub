...
if(isset($_POST['import'])){
	...
	if(!empty($tmpName)){
		copy($tmpName, "$upload_dir[basedir]/tubeace-xhamster.csv");
	} else {
	...
	if(empty($error)){
        ...
        $fh = fopen("$upload_dir[basedir]/tubeace-xhamster.csv", 'r+');
        $content = fread($fh, filesize("$upload_dir[basedir]/tubeace-xhamster.csv"));
        fclose($fh);
        ...
        $fieldArr = array('video_id','thumbs','url_thumb','title','categories','duration');
              ...
              flush();  
                ...
                if(@!$errorInsert){   
					...
					if($lastID = wp_insert_post( $my_post )){
						...
						add_post_meta($lastID, 'site', 'xhamster.com');
...
?>
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-xhamster.php'); ?>" method="post" enctype="multipart/form-data">
