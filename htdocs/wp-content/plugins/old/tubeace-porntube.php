...
if(isset($_POST['import'])){
	...
	if(!empty($tmpName)){
		copy($tmpName, "$upload_dir[basedir]/tubeace-porntube.txt");
	} else {
	...
	if(empty($error)){
        ...
        $fh = fopen("$upload_dir[basedir]/tubeace-porntube.txt", 'r+');
        $content = fread($fh, filesize("$upload_dir[basedir]/tubeace-porntube.txt"));
        fclose($fh);
        ...
        $fieldArr = array('video_id','url','title','default_thumb','thumbs','duration','pornstars','categories','added','tags','height');
        ...
        foreach($lines as $lines_key => $lines_val){
            ...
            foreach($fields = explode("|", $lines_val) as $fields_key => $fields_val) {
                ...
                if($fieldArr[$fields_key]=="pornstars"){
            ...
            if ($num_rows > 0){
              ...
              echo"<span class=\"tubeace-errormsg\">Video skipped! Video titled <b>'$title'</b> Thumbs field empty.</span><br>";
              flush();  
            } else {            
                ...
                foreach($thumbs as $val){
                  ...
                  if(!@getimagesize($val) && !@$errorInsert){
                    echo"<span class=\"tubeace-errormsg\">Video '$title' skipped - Thumbnail URL invalid!</span>";
                    flush();
                    $errorInsert=1;
                  }
                } 
                ...
                if(@!$errorInsert){   
					...
					if($lastID = wp_insert_post( $my_post )){
						...
						add_post_meta($lastID, 'site', 'porntube.com');
                    ...
                    foreach($thumbs as $val){
						...
						copy($val,$thumbDest);
							...
							tubeace_resize_thumb($val,$thumbDest,'jpg',$width,$height,320,240);
						}
						$i++;
                    } 
...
?>
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-porntube.php'); ?>" method="post" enctype="multipart/form-data">
