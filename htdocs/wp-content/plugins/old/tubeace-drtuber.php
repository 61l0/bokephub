...
if(isset($_POST['import'])){
	...
	if(!empty($tmpName)){
		copy($tmpName, "$upload_dir[basedir]/tubeace-drtuber.csv");
	} else {
	...
	if(empty($error)){
        ...
        $fh = fopen("$upload_dir[basedir]/tubeace-drtuber.csv", 'r+');
        $content = fread($fh, filesize("$upload_dir[basedir]/tubeace-drtuber.csv"));
        fclose($fh);
        ...
        $fieldArr = array('video_id','url','thumbs','title','categories','duration');
              ...
              flush();  
                ...
                if(@!$errorInsert){   
					...
					if($lastID = wp_insert_post( $my_post )){
						...
						add_post_meta($lastID, 'site', 'drtuber.com');
...
?>
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-drtuber.php'); ?>" method="post" enctype="multipart/form-data">
		...
		<table class="form-table">
		    <tbody>
		        ...
		        <tr>
		    		<th></th>
		        	<td>
		        		<input type="hidden" name="page" value="tubeace/tubeace-drtuber.php">
