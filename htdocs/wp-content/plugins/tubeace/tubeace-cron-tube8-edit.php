...
if( isset($_POST['Submit']) ){
	...
	if($current_frequency != $cron_frequency){
		...
		$existingCronNames = get_site_option('tubeace_cron_tube8_'.$current_frequency);
		...
		if(count($existingCronNames)>1){
			...
			update_site_option("tubeace_cron_tube8_".$current_frequency, $existingCronNames);
		
		} else { //only one name, delete from options and deactivate group cron hook

			//delete empty group option_name
			$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_tube8_".$current_frequency ) );
		...
		$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_tube8_".$cron_name."_".$current_frequency ) );

		//create new frequency group, or add to existing
		$newCronNames = get_site_option('tubeace_cron_tube8_'.$cron_frequency);
...
$option_values = get_site_option('tubeace_cron_tube8_'.$cron_name.'_'.$cron_frequency);

?>

<h2>Edit Tube8 Import Cron Job</h2>

<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-cron-tube8-edit.php'); ?>" method="POST">
