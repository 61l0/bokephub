<?php
		...
		$outputAll="";
	  	...
	  	for($page=$start;$page<=$end;$page++){
	  		...
	  		$call = "http://api.tube8.com/api.php?action=searchVideos&output=json&search=$keyword&thumbsize=big&page=$page";
			...
			foreach($videos as $vid) {
				...
				$title = addslashes($vid->title);
				$url = $video_obj->url;
				$video_id = $vid->video->video_id;

				//reset
				$tags="";
				//perfomer not in API output, so checkfor Set All
					...
					if(!$errorInsert){
						...
						foreach($vid->thumbs->big as $val){
							...
							if($width==get_site_option('tubeace_thumb_width') && $height==get_site_option('tubeace_thumb_height')){
								...
								tubeace_resize_thumb($val,$thumbDest,'jpg',$width,$height,get_site_option('tubeace_thumb_width'),get_site_option('tubeace_thumb_height'));	
							}
