...
if(!empty($_POST['Delete-pornhub'])) {
	...
	foreach( $id1 as $cron_name) {
		...
		if($del[$cron_name]==1){
			...
			$existingCronNames = get_site_option('tubeace_cron_pornhub_'.$frequency);
			...
			if(count($existingCronNames)>1){
				...
				update_site_option("tubeace_cron_pornhub_".$frequency, $existingCronNames);
			
			} else { //only one name, delete from options and deactivate group cron hook


				//delete empty group option_name
				$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_pornhub_".$frequency ) );
			...
			$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_pornhub_".$cron_name."_".$frequency ) );
...
<h3>Pornhub Import Cron Jobs</h3>
...
<table class="widefat" cellspacing="0">
  <thead>
	...
	<tbody>
		...
		//hourly
		//get options array
		$existingCronNamesHourly = get_site_option('tubeace_cron_pornhub_hourly');
		...
		if(!empty($existingCronNamesHourly)){
			...
			foreach($existingCronNamesHourly as $name){
				...
				if(!in_array($name, $deleted_cron_names)){
				    ...
				    $option_values = get_site_option('tubeace_cron_pornhub_'.$name.'_hourly');
					...
					<tr class=\"alternate\">
						...
						<td>Hourly <br><a href=\"admin.php?page=tubeace/tubeace-cron-pornhub-edit.php&freq=hourly&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$start</td>
						<td>$end</td>
		...
		//twicedaily
		//get options array
		$existingCronNamesTwice = get_site_option('tubeace_cron_pornhub_twicedaily');
		...
		if(!empty($existingCronNamesTwice)){
			...
			foreach($existingCronNamesTwice as $name){
				...
				if(!in_array($name, $deleted_cron_names)){				
				    ...
				    $option_values = get_site_option('tubeace_cron_pornhub_'.$name.'_twicedaily');
					...
					<tr class=\"alternate\">
						...
						<td>Twice Daily <br><a href=\"admin.php?page=tubeace/tubeace-cron-pornhub-edit.php&freq=twicedaily&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$start</td>
						<td>$end</td>
		...
		//daily
		//get options array
		$existingCronNamesDaily = get_site_option('tubeace_cron_pornhub_daily');
		...
		if(!empty($existingCronNamesDaily)){
			...
			foreach($existingCronNamesDaily as $name){
				...
				if(!in_array($name, $deleted_cron_names)){
				    ...
				    $option_values = get_site_option('tubeace_cron_pornhub_'.$name.'_daily');
					...
					<tr class=\"alternate\">
						...
						<td>Daily <br><a href=\"admin.php?page=tubeace/tubeace-cron-pornhub-edit.php&freq=daily&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$start</td>
						<td>$end</td>
...
<input type="submit" value="Delete Selected" class="button-primary" name="Delete-pornhub">
</form>