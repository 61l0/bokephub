...
if(!empty($_POST['Delete-redtube'])) {
	...
	if (is_array($id1)) {
		...
		foreach( $id1 as $cron_name) {
			...
			if($del[$cron_name]==1){
				...
				$existingCronNames = get_site_option('tubeace_cron_redtube_'.$frequency);
				...
				if(count($existingCronNames)>1){
					...
					update_site_option("tubeace_cron_redtube_".$frequency, $existingCronNames);
				
				} else { //only one name, delete from options and deactivate group cron hook


					//delete empty group option_name
					$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_redtube_".$frequency ) );
				...
				$wpdb->delete( $wpdb->prefix.'options', array( 'option_name' => "tubeace_cron_redtube_".$cron_name."_".$frequency ) );
	...
	if($i>0){
		...
		echo"<div class=\"error\"><p><b>No Cron Jobs selected to delete.</b></p></div>";
	}
}

?>

<h3>RedTube Import Cron Jobs</h3>
...
<table class="widefat" cellspacing="0">
  <thead>
	...
	<tbody>
		...
		//hourly
		//get options array
		$existingCronNamesHourly = get_site_option('tubeace_cron_redtube_hourly');
		...
		if(!empty($existingCronNamesHourly)){
			...
			foreach($existingCronNamesHourly as $name){
				...
				if(!in_array($name, $deleted_cron_names)){
				    ...
				    $option_values = get_site_option('tubeace_cron_redtube_'.$name.'_hourly');
					...
					<tr class=\"alternate\">
						...
						<td>Hourly <br><a href=\"admin.php?page=tubeace/tubeace-cron-redtube-edit.php&freq=hourly&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$tag</td>
						<td>$start</td>
		...
		//twicedaily
		//get options array
		$existingCronNamesTwice = get_site_option('tubeace_cron_redtube_twicedaily');
		...
		if(!empty($existingCronNamesTwice)){
			...
			foreach($existingCronNamesTwice as $name){
				...
				if(!in_array($name, $deleted_cron_names)){				
				    ...
				    $option_values = get_site_option('tubeace_cron_redtube_'.$name.'_twicedaily');
					...
					<tr class=\"alternate\">
						...
						<td>Twice Daily <br><a href=\"admin.php?page=tubeace/tubeace-cron-redtube-edit.php&freq=twicedaily&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$tag</td>
						<td>$start</td>
		...
		//daily
		//get options array
		$existingCronNamesDaily = get_site_option('tubeace_cron_redtube_daily');
		...
		if(!empty($existingCronNamesDaily)){
			...
			foreach($existingCronNamesDaily as $name){
				...
				if(!in_array($name, $deleted_cron_names)){
				    ...
				    $option_values = get_site_option('tubeace_cron_redtube_'.$name.'_daily');
					...
					<tr class=\"alternate\">
						...
						<td>Daily <br><a href=\"admin.php?page=tubeace/tubeace-cron-redtube-edit.php&freq=daily&name=$name\">Edit</a> </td>
						<td>$name</td>
						<td>$keyword</td>
						<td>$tag</td>
						<td>$start</td>
...
<input type="submit" value="Delete Selected" class="button-primary" name="Delete-redtube">
</form>