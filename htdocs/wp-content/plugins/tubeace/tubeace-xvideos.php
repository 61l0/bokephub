...
if(isset($_POST['import'])){
	...
	if(!empty($tmpName)){
		copy($tmpName, "$upload_dir[basedir]/tubeace-xvideos.csv");
	} else {
	...
	if(empty($error)){
        ...
        $fh = fopen("$upload_dir[basedir]/tubeace-xvideos.csv", 'r+');
        $content = fread($fh, filesize("$upload_dir[basedir]/tubeace-xvideos.csv"));
        fclose($fh);
        ...
        $fieldArr = array('video_url','title','duration','thumb','embed_code','tags','categories');
              ...
              flush();  
                ...
                if(@!$errorInsert){   
					...
					if($lastID = wp_insert_post( $my_post )){
						...
						add_post_meta($lastID, 'site', 'xvideos.com');
...
?>
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-xvideos.php'); ?>" method="post" enctype="multipart/form-data">
		...
		<table class="form-table">
		    <tbody>
		        ...
		        <tr>
		    		<th></th>
		        	<td>
		        		<input type="hidden" name="page" value="tubeace/tubeace-xvideos.php">
