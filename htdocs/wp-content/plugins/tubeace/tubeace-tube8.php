...
if(isset($_POST['import'])){
	...
	if(!empty($tmpName)){
		copy($tmpName, "$upload_dir[basedir]/tubeace-tube8.csv");
	} else {
	...
	if(empty($error)){
        ...
        $fh = fopen("$upload_dir[basedir]/tubeace-tube8.csv", 'r+');
        $content = fread($fh, filesize("$upload_dir[basedir]/tubeace-tube8.csv"));
        fclose($fh);
...
?>
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-tube8.php'); ?>" method="post" enctype="multipart/form-data">
		...
		<table class="form-table">
		    <tbody>
		        ...
		        <tr>
		    		<th></th>
		        	<td>
		        		<input type="hidden" name="page" value="tubeace/tubeace-tube8.php">
