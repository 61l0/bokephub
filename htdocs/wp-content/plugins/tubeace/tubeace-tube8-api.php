...
if($_POST['m']=="single"){ 
	...
	echo"
	<div class=\"wrap\">
		<h2>Tube8 Import - Single Video</h2>";
		...
		if($num_rows>0){
			echo"<span class=\"tubeace-errormsg\">WARNING: Video already exists in database!</span>";
		}

		echo"
		<form action=\"".admin_url('admin.php?page=tubeace/tubeace-tube8-api.php')."\" method=\"post\">
			...
			<table class=\"form-table\">
			    <tbody>
			        ...
			        <tr>
			        	...
			        	<td>
			        	  <input type=\"text\" class=\"input-400\" name=\"performers\" value=\"\">
						</td>		        	
			        </tr>	
			        <tr>
...
if($_POST['m']=="import"){
	...
	add_post_meta($lastID, 'site', 'tube8.com');
	...
	$data = file_get_contents("http://api.tube8.com/api.php?action=getvideobyid&video_id=$video_id&output=json&thumbsize=big");
	...
	foreach($data->thumbs->big as $val){
		
		list($width, $height) = getimagesize($val);
		
		$thumbDest = $upload_dir['basedir']."/tubeace-thumbs/".$subPath."/".$lastID."_".$i.".jpg";

		tubeace_resize_thumb($val,$thumbDest,'jpg',$width,$height,$tubeace_thumb_width,$tubeace_thumb_height);
		$i++;
	}	
...
if($_GET['cron']){
	...
	$existingCronNames = get_site_option('tubeace_cron_tube8_'.$cron_frequency);
	...
	//if none exist, add new
		...
		update_site_option('tubeace_cron_tube8_'.$cron_frequency, $cronNames);	
		update_site_option("tubeace_cron_tube8_".$cron_name."_".$cron_frequency, $cronValues);

		echo"<div class=\"updated\"><p>Added $cron_frequency cron named '$cron_name'.</p></div>";

	} else { //if already exists, we need to add name to tubeace_cron_tube8_{frequency}
...
<div class="wrap">
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-tube8-api.php'); ?>" method="post">
		<table class="form-table">
		    <tbody>
		        <tr>
		    		<th><label for="video_id">Tube8 Video ID</label></th>
		        	<td>
		        ...
		        <tr>
		    		<td></td>
		        	<td>
		        		<input type="hidden" name="page" value="tubeace/tubeace-tube8-api.php">
	...
	<h3>Search Videos</h3>
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-tube8-api.php'); ?>" method="get">
		<table class="form-table">
		    <tbody>
		        <tr>
