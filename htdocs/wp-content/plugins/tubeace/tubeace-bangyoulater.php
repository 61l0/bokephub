...
if(isset($_POST['import'])){
	...
	if(!empty($tmpName)){
		copy($tmpName, "$upload_dir[basedir]/tubeace-bangyoulater.csv");
	} else {
	...
	if(empty($error)){
        ...
        $fh = fopen("$upload_dir[basedir]/tubeace-bangyoulater.csv", 'r+');
        $content = fread($fh, filesize("$upload_dir[basedir]/tubeace-bangyoulater.csv"));
        fclose($fh);
        ...
        $fieldArr = array('video_id','categories','title','duration','description','thumbnail','big_thumbnail','video_url','embed_code');
        ...
        foreach($lines as $lines_key => $lines_val){
            ...
            foreach($fields = explode("|", $lines_val) as $fields_key => $fields_val) {
                ...
                if($fieldArr[$fields_key]=="description"){
                    $description = $fields_val_con;
                }                
                if($fieldArr[$fields_key]=="big_thumbnail"){
              ...
              flush();  
                ...
                if(@!$errorInsert){   
					...
					if($lastID = wp_insert_post( $my_post )){
						...
						add_post_meta($lastID, 'site', 'bangyoulater.com');
...
?>
	...
	<form action="<?php echo admin_url('admin.php?page=tubeace/tubeace-bangyoulater.php'); ?>" method="post" enctype="multipart/form-data">
		...
		<table class="form-table">
		    <tbody>
		        ...
		        <tr>
		    		<th></th>
		        	<td>
		        		<input type="hidden" name="page" value="tubeace/tubeace-bangyoulater.php">
